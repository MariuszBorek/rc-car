...
# ===== KONFIG =====
PORT = 5000
PUSHOVER_USER_KEY = "KEY"        # <-- Twój Pushover User Key
PUSHOVER_APP_TOKEN = "TOKEN"     # <-- Twój Pushover App Token
PUSHOVER_RETRIES = 10
...
def send_pushover(message, retries=PUSHOVER_RETRIES, delay=PUSHOVER_DELAY):
    for _ in range(retries):
        try:
            ...
            print(f"Pushover error, retrying in {delay}s: {e}")
            time.sleep(delay)
...
def start_camera():
    ...
    try:
        camera = Picamera2()
        # Stała konfiguracja przy starcie – 640x480 dla stabilności na powerbanku
...
if __name__ == "__main__":
    start_camera()
    time.sleep(5)  # poczekaj chwilę na sieć
